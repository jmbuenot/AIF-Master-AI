...
# Explicación: adapta las etiquetas human-readable al nuevo orden de 
# orientaciones especificado por el enunciado.
...
# Explicación: describe un nodo del árbol de búsqueda con sus métricas y ofrece 
# `to_tuple` para formatear la salida.
...
# 5. `raise ValueError(...)` lanza una excepción si el fichero no contiene los bloques mínimos requeridos.
    ...
    #Tras la matriz decodifica el estado de entrada y el estado de salida
...
#Recordemos que la matriz de la malla/mapa es cuadrada 
...
# Explicación: lista giros y movimientos factibles; al avanzar traduce 
# el vector cartesiano (dx, dy) de la orientación al desplazamiento (fila, columna) y descarta pasos que salgan del mapa.
...
# Explicación: muestra la matriz numerada por filas y resume las coordenadas 
# iniciales y finales usando etiquetas de orientación legibles.
...
def solve_from_file(
    ...
    *,
    ...
    """Lee un problema desde disco, opcionalmente lo muestra y ejecuta la BFS."""
...
# Explicación: función de alto nivel que encapsula la lectura del fichero, 
# el despliegue del mapa y la invocación de la BFS devolviendo todos los resultados necesarios.
...
def bfs(grid: List[List[int]], start: State, goal: State):
    ...
    # Explicación: `frontier` es una cola FIFO (`deque`) que mantiene el orden por profundidad de los nodos
    # pendientes de expansión. Tras inicializarla con el nodo raíz, el conjunto auxiliar
    # `frontier_states` almacena las tuplas (fila, columna, orientación) presentes en la cola para
    # poder comprobar duplicados en O(1) sin tener que recorrerla; ambos se actualizan de forma
    # coordinada a medida que se extraen y añaden nodos durante la búsqueda.
    ...
    while frontier:
        ...
        # Explicación: `discard` elimina el estado de la frontera si 
        # está presente y evita excepciones si ya no figura en el conjunto.
...
def reconstruct_path(node: Node) -> List[Node]:
    ...
    path.reverse()
    return path
# Explicación: recompone la secuencia de nodos 
# siguiendo los punteros al padre desde la meta hasta el inicio.
...
def print_trace_bfs(path: List[Node], frontier: List[Node], explored_count: int) -> None:
    ...
    print()
    print(f"Elementos en la lista de explorados: {explored_count}")
    print(f"Total number of items in frontier: {len(frontier)}")

# Explicación: recompone la secuencia de 
# nodos siguiendo los punteros al padre desde la meta hasta el inicio.
...
# Explicación: delega la lectura del fichero, el despliegue del mapa
#  y la ejecución de la BFS en `solve_from_file` para reutilizar la 
# lógica y asegurar el uso de `print_problem_overview`.
...
# Explicación: permite ejecutar el módulo como script invocando 
# `main` con los argumentos de la línea de comandos.
